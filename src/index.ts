...
export function apply(ctx: Context, cfg: Config) {
			...
			try {
				if (
					...
					if (cfg.ChannelName_Setting === true) {
						var ChannelName = session.channelName || session.guildName;
						if (!ChannelName) {
							if (typeof session.bot.getChannel === "function") {
								ChannelName = (
									await session.bot.getChannel(session.channelId)
								).name;
							}
						}
					}
	...
	try {
		...
		for (let f = 0; f < cfg.Forward_Groups.length; f++) {
			for (let i = 0; i < cfg.Forward_Groups[f].Forward_Node.length; i++) {
				for (let o = i + 1; o < cfg.Forward_Groups[f].Forward_Node.length; o++) {
